...
def adversarial_measurement(
    ...
    # robust_qubits = random.sample(range(qubits), corruption)
...
def growing_power_experiment_with_correction(
    ...
    # ax.plot(qubit_count, success_rate, linewidth=2.0)
    cos_squared = [
        1 / qubits + ((qubits - 1) / qubits) * (math.cos(p / 2) ** 2) ** corruption
        for p in powers  # TODO crazy hack
    ]
...
if __name__ == "__main__":
    ...
    growing_power_experiment_with_correction(
        0, 3.14, step=0.157, corruption=1, qubits=8, runs=500, shots=1
    )
